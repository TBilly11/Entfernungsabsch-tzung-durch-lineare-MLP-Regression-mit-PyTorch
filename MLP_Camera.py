...
X = data_array[:, :-1]  # Weite, Höhe, Zoom
...
# Modell definieren
model = nn.Sequential(
    nn.Linear(3, 10),  # Eingänge: Weite, Höhe, Zoom
...
'''model = nn.Sequential(
    nn.Linear(3, 20),    # Erhöhung der Anzahl der Neuronen in der ersten Schicht
    nn.ReLU(),
    nn.Dropout(0.2),   # Dropout zwischen den Schichten
...
'''model = nn.Sequential(
    nn.Linear(3, 30),    # Erste Schicht: Eingang mit 3 Merkmalen zu 64 Neuronen
    nn.ReLU(),           # Aktivierungsfunktion
    nn.Dropout(0.2),   # Dropout zwischen den Schichten
